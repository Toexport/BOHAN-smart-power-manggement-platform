...
#define ZPFormat(format, ...)   [NSString stringWithFormat:format, ##__VA_ARGS__]
#ifndef ZPSCREEN_WIDTH
#define ZPSCREEN_WIDTH [UIScreen mainScreen].bounds.size.width
#endif

@interface KeyBoard ()

@property (nonatomic, assign) ZPKBType type;
...
+ (instancetype)keyboardWithType:(ZPKBType)type {
   return [[KeyBoard alloc] initWithFrame:CGRectMake(0, 0, ZPSCREEN_WIDTH, ZPKBH+ZPICONH) withType:type];
}

- (id)initWithFrame:(CGRect)frame withType:(ZPKBType)type {
...
- (CGFloat)widthForInfo:(NSString *)info {
    ...
    NSDictionary *attributs = [NSDictionary dictionaryWithObjectsAndKeys:ZPKBFont(ZPKBFontSize-5), NSFontAttributeName, nil];
...
- (void)setEnterprise:(NSString *)enterprise {
    if (enterprise.length > 0) {
        ...
        CGFloat iconH = ZPICONH;
        CGFloat icon_w_h = iconH*0.5;
        CGFloat width = [self widthForInfo:enterprise];
        CGFloat wwww = icon_w_h*1.5+width;
        CGFloat start_x = (ZPSCREEN_WIDTH-wwww)*0.5+icon_w_h*1.5;
        CGRect bounds = CGRectMake(start_x, ZPICONH*0.25, width, ZPICONH*0.5);
...
- (UILabel *)iconLabel {
    if (!_iconLabel) {
        ...
        _iconLabel.font = ZPKBFont(ZPKBFontSize-5);
...
- (UIImageView *)iconFlag {
    if (!_iconFlag) {
        ...
        _iconFlag.layer.cornerRadius = ZPICONH*0.25;
...
- (void)_initSetup {
    //默认信息
    self.icon = @"logo";
    self.enterprise =Localize(@"伯瀚安全输入");
    CGFloat iconH = ZPICONH;
    CGFloat kbH = ZPKBH;
    ...
    //seperate line
    CGFloat lineH = 1;
    bounds = CGRectMake(0, ZPICONH-lineH, ZPSCREEN_WIDTH,lineH);
    UIView *line = [[UIView alloc] initWithFrame:bounds];
    line.backgroundColor = NHColor(101, 101, 101);
    [self addSubview:line];
    
    //创建键盘
    if (ZPKBTypeNumberPad == self.type) {
        [self reloadNumberPad:false];
    }else if (ZPKBTypeDecimalPad == self.type){
        [self reloadNumberPad:true];
    }else if (ZPKBTypeASCIICapable == self.type){
...
- (void)willMoveToWindow:(UIWindow *)newWindow {
    ...
    if (!newWindow && self.type != ZPKBTypeASCIICapable) {
...
- (void)reloadNumberPad:(BOOL)decimal {
    if (self.type != ZPKBTypeASCIICapable) {
        ...
        UIFont *titleFont = ZPKBFont(ZPKBFontSize);
        CGFloat itemH = ZPKBH/rows;
        CGFloat itemW = ZPSCREEN_WIDTH/cols;
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                CGRect bounds = CGRectMake(j*itemW, i*itemH+ZPICONH, itemW, itemH);
...
- (void)loadRandomNumber {
    BOOL decimal = (self.type == ZPKBTypeDecimalPad);
...
- (void)setupASCIICapableLayout:(BOOL)init {
    if (!init) {
        ...
        NSArray *subviews = self.subviews;
        [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[ZPChar class]]) {
    ...
    CGFloat char_width = (ZPSCREEN_WIDTH-char_h_dis*len)/len;
    CGFloat char_heigh = (ZPKBH-char_uper_dis*2-char_v_dis*3)/4;
    UIFont *titleFont = ZPKBFont(ZPKBFontSize);
    UIColor *titleColor = NHColor(247, 247, 247);
    UIColor *bgColor = NHColor(64, 66, 68);
    UIImage *bgImg = [UIImage pb_imageFromColor:bgColor];
    CGFloat cur_y = ZPICONH+char_uper_dis;
    
    int n = 0;
     UIImage *charbgImg = [bgImg pb_drawRectWithRoundCorner:ZPCHAR_CORNER toSize:CGSizeMake(char_width, char_heigh)];
    for (int i = 0 ; i < len; i ++) {
        ...
        ZPChar *btn = [ZPChar buttonWithType:UIButtonTypeCustom];
        btn.frame = bounds;
        //btn.chars = title;
        btn.exclusiveTouch = true;
        //消耗性能
    ...
    //NSLog(@"第二排:%@",chars);
    len = [chars count];
    CGFloat start_x = (ZPSCREEN_WIDTH-char_width*len-char_h_dis*(len-1))/2;
    for (int i = 0 ; i < len; i ++) {
    ...
    char_width = (ZPSCREEN_WIDTH-char_h_dis*4-shiftWidth*2-char_h_dis*(len-1))/len;
//    charbgImg = [bgImg pb_drawRectWithRoundCorner:NHCHAR_CORNER toSize:CGSizeMake(char_width, char_heigh)];
    ...
    if (init) {
        UIImage *roundImg = [bgImg pb_drawRectWithRoundCorner:ZPCHAR_CORNER toSize:CGSizeMake(shiftWidth, char_heigh)];
        ...
        bounds = CGRectMake(ZPSCREEN_WIDTH-char_h_dis*0.5-shiftWidth, cur_y, shiftWidth, char_heigh);
        btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = bounds;
        btn.exclusiveTouch = true;
        //btn.backgroundColor = NHColor(64, 66, 68);
    ...
    if (init) {
        ...
        UIImage *roundImg = [bgImg pb_drawRectWithRoundCorner:ZPCHAR_CORNER toSize:CGSizeMake(numWidth, char_heigh)];
        ...
        bounds = CGRectMake(ZPSCREEN_WIDTH-char_h_dis*0.5-numWidth, cur_y, numWidth, char_heigh);
        btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = bounds;
        btn.exclusiveTouch = true;
        //btn.backgroundColor = NHColor(64, 66, 68);
        ...
        CGFloat spaceWidth = (ZPSCREEN_WIDTH-char_h_dis*3-numWidth*2);
...
- (void)loadCharacters:(NSArray *)array {
    ...
    NSArray *subviews = self.subviews;
    [subviews enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj && [obj isKindOfClass:[ZPChar class]]) {
            ZPChar *tmp = (ZPChar *)obj;
            NSInteger __tag = tmp.tag;
...
- (void)characterTouchAction:(ZPChar *)btn {
    NSString *title = [btn titleLabel].text;
    if (self.inputSource) {
        if ([self.inputSource isKindOfClass:[UITextField class]]) {
            UITextField *tmp = (UITextField *)self.inputSource;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    for (ZPChar *tmp in self.charsBtn) {
...
- (void)addPopup {
    ...
    if ([self.chars isEqualToString:@"q"]||[self.chars isEqualToString:@"a"]) {
        ...
        keyPop = [[UIImageView alloc] initWithImage:[UIImage imageWithCGImage:[self createKeytopImageWithKind:ZPKBImageInner] scale:[[UIScreen mainScreen] scale] orientation:UIImageOrientationDown]];
        keyPop.frame = CGRectMake(-27, -71, keyPop.frame.size.width, keyPop.frame.size.height);
    }
    NSString *tmp = self.isShift?[self.chars uppercaseString]:[self.chars lowercaseString];
    [text setFont:[UIFont fontWithName:ZPKBFont(ZPKBFontSize).fontName size:44]];
