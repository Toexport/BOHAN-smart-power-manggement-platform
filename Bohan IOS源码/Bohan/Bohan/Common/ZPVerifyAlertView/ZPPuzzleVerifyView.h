...
@class ZPPuzzleVerifyView;
NS_ASSUME_NONNULL_BEGIN

typedef NS_ENUM(NSInteger, ZPPuzzleVerifyStyle) {
    ZPPuzzleVerifyStyleClassic,   // 默认
    ZPPuzzleVerifyStyleSquare,    // 矩形
    ZPPuzzleVerifyStyleCircle,    // 圆形
    ZPPuzzleVerifyStyleCustom     // 自定义
};

@interface ZPPuzzleVerifyView : UIView
...
/** 滑块风格（具体指滑块的形状） */
@property (nonatomic, assign) ZPPuzzleVerifyStyle style;
/** 滑块的大小（默认 40.f）*/
...
/** 指定初始化 */
- (instancetype)initWithFrame:(CGRect)frame style:(ZPPuzzleVerifyStyle)style NS_DESIGNATED_INITIALIZER;
