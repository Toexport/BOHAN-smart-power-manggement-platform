...
- (CAAnimation *)dimingAnimation
{
    if (_dimingAnimation == nil) {
        ...
        [_dimingAnimation setRemovedOnCompletion:NO];
        [_dimingAnimation setFillMode:kCAFillModeBoth];
    }
...
- (CAAnimation *)lightingAnimation
{
    if (_lightingAnimation == nil ) {
        ...
        [_lightingAnimation setRemovedOnCompletion:NO];
        [_lightingAnimation setFillMode:kCAFillModeBoth];
    }
...
- (CAAnimation *)showMenuAnimation
{
    if (_showMenuAnimation == nil) {
        ...
        [positionAnimation setFromValue:[NSNumber numberWithFloat:246.0]];
        ...
        _showMenuAnimation = positionAnimation;
        
        [_showMenuAnimation setRemovedOnCompletion:NO];
        [_showMenuAnimation setFillMode:kCAFillModeBoth];
    }
...
- (CAAnimation *)dismissMenuAnimation
{
    if (_dismissMenuAnimation == nil) {
        ...
        [positionAnimation setToValue:[NSNumber numberWithFloat:246.0]];
        ...
        [_dismissMenuAnimation setRemovedOnCompletion:NO];
        [_dismissMenuAnimation setFillMode:kCAFillModeBoth];
    }
...
- (void)layoutSubviews {
    ...
    SGBaseMenu *topMenu = self.menus.lastObject;
    topMenu.frame = CGRectMake(0, self.bounds.size.height - topMenu.bounds.size.height, [UIScreen mainScreen].bounds.size.width, topMenu.bounds.size.height);
    //    SGSheetMenu *sheetMenu = self.menus.lastObject;
    //    sheetMenu.frame = CGRectMake(0, self.bounds.size.height - sheetMenu.bounds.size.height, [UIScreen mainScreen].bounds.size.width, sheetMenu.bounds.size.height);
    //    [sheetMenu setNeedsLayout];
    [topMenu setNeedsLayout];
}
           ...
           selectedHandle:(SGDateTimePick)handler
{
    ...
    [menu triggerSelectedAction:^(NSDate *selectTime,NSString *selectedItem){
        [[SGActionView sharedActionView] dismissMenu:menu Animated:YES];
        if (handler && (selectTime!=nil || selectedItem!= nil))
        {
            handler(selectTime,selectedItem);
        }
    }];
    [menu setDataArray:[[SGActionView sharedActionView] dataSource]];
